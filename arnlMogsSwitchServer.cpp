...
class LocSwitchAtGoal
{
private:
  ArPathPlanningInterface *myPathTask;
  LocSwitcher *myLocSwitch;
  std::string myLaserGoalNamePrefix;
  std::string myGPSGoalNamePrefix;
  ArFunctor1C<LocSwitchAtGoal, ArPose> myGoalCB;

public:
  LocSwitchAtGoal(ArPathPlanningInterface *pathTask, LocSwitcher *locSwitch, const std::string& gpsGoalNamePrefix, const std::string& laserGoalNamePrefix) : 
    myPathTask(pathTask),
    myLocSwitch(locSwitch), 
    myLaserGoalNamePrefix(laserGoalNamePrefix), 
    myGPSGoalNamePrefix(gpsGoalNamePrefix), 
    myGoalCB(this, &LocSwitchAtGoal::goalReached)
  {
    pathTask->addGoalDoneCB(&myGoalCB);
    ArLog::log(ArLog::Normal, "LocSwitchAtGoal: will check goals with names beginning \"%s...\" and \"%s...\"", myLaserGoalNamePrefix.c_str(), myGPSGoalNamePrefix.c_str());
  }
...
private:
  void goalReached(ArPose pose)
  {
    ...
    if(goalname.compare(0, myGPSGoalNamePrefix.size(), myGPSGoalNamePrefix) == 0)
    {
      ArLog::log(ArLog::Normal, "LocSwitchAtGoal: switching to GPS...");
      myLocSwitch->switchToGPSAsync();
    }
    else if(goalname.compare(0, myLaserGoalNamePrefix.size(), myLaserGoalNamePrefix) == 0)
    {
      ArLog::log(ArLog::Normal, "LocSwitchAtGoal: switching to Laser...");
      myLocSwitch->switchToLaserAsync();
    }
  ...
  //myLaserSlowDownAction(laserSlowDownAct), myGPSSlowDownAction(gpsSlowDownAct)
{
  if(cmdSrv)
  {
    cmdSrv->addCommand("0 Switch To Laser Localization", "Switch To Laser Localization", 
      new ArRetFunctor1C<bool, LocSwitcher, int>(this, &LocSwitcher::switchToLaser, -1));
    cmdSrv->addCommand("0 Switch To GPS Localization", "Switch To GPS Localization", 
